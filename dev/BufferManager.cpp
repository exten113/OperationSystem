...
void BufferManager::logInitialize()
{
	...
	if (!(log_flag&=Buf::B_TXB))//判断如果TxB=0，则无日志，退出
		return;
	else if (log_flag&=Buf::B_CHECKPT)//判断如果B_CHECKPT=1，则上次成功写入，退出
		return;
	else if (!(log_flag&=Buf::B_TXE))//判断如果B_TxE=0，则日志不是有效日志，退出
		return;
...
void BufferManager::WriteLog(){
	...
	for(i=0;i<15;i++){
		if(this->m_Buf[i].b_flags&Buf::B_LOG){
			...
			m_Buf[i].b_flags &= ~(Buf::B_LOG);/*写过日志了*/
			BufferManager::Brelse(&this->m_Buf[i]);/*该脏块可以释放了，进入自由队列*/
		}
	}
