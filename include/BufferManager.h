...
public:  	
	...
	void logInitialize();				/* 对日志缓存进行初始化，并读取硬盘中的日志来实现对错误的回滚*/
...
private:
	...
	void WriteLog();
	Buf* ReadLog(short dev, int blkno);	/* 读取日志内容*/
	void Write(Buf *bp);				/*进行一个实际的写*/
private:
	...
	Buf logBuf; //控制写入的额外信息
	unsigned char logBuffer[BUFFER_SIZE];//额外信息块
